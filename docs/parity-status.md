...
| 10 | Original asset loader | partial | `src/assets/AssetSource.hpp`; `OriginalPiAssetSource.*`; `FallbackAssetSource.*`; repository ships no Mojang assets | `asset_source` + `assets_cli` (local reads, traversal rejection, explicit `--assets` CLI path, missing-file fallback; exact original asset lookup hierarchy remains partial) |
